...
for i in [32, 64]:
  with open(dir_name + '/'+str(i)+'.mcfunction', mode='w') as f:
    ...
    for j in range(-i, 0):
      s += 'execute if data storage bitset.__temp__: __input__.bitset1.data['+str(j)+']._{_: true} if data storage bitset.__temp__: __input__.bitset2.data['+str(j)+']._{_: true} run data modify storage bitset.__temp__: __output__.bitset.data['+str(j)+']._._ set value true\n'
